...
pub fn skp_silk_nlsf_stabilize(mut nlsf_q15: &mut [i32], n_delta_min_q15: &[i32], len: usize) {
    let mut loops = 0;
    while loops < 20 {
        let mut min_diff_q15 = nlsf_q15[0] - n_delta_min_q15[0];
        let mut i_len = 0;
        for i in 1..len {
            let diff_q15 = nlsf_q15[i] - (nlsf_q15[i - 1] + n_delta_min_q15[i]);
            if diff_q15 < min_diff_q15 {
                min_diff_q15 = diff_q15;
                i_len = i;
            }
        }
        let diff_q15 = (1 << 15) - (nlsf_q15[len - 1] + n_delta_min_q15[len]);
        if diff_q15 < min_diff_q15 {
            min_diff_q15 = diff_q15;
            i_len = len;
        }
        ...
        if i_len == 0 {
            ...
            let center_freq_q15 = skp_limit_32!(
