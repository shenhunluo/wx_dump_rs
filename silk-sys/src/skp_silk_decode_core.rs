...
pub fn skp_silk_decode_core(
    ...
    let offset_q10 = SKP_SILK_QUANTIZATION_OFFSETS_Q10[ps_dec_ctrl.sig_type as usize]
        [ps_dec_ctrl.QuantOffsetType as usize] as i32;
    let nlsf_interpolation_flag = if ps_dec_ctrl.NLSFInterpCoef_Q2 < 1 << 2 {
        1
    } else {
        0
    };
    ...
    for k in 0..4 {
            ...
            && k < 4 >> 1
        {
            ...
            b_q14[5 / 2] = 1 << 12;
            sig_type = 0;
            ps_dec_ctrl.pitchL[k as usize] = ps_dec.lagPrev;
        }
        if sig_type == 0 {
            lag = ps_dec_ctrl.pitchL[k as usize];
            if k as i32 & 3 - skp_l_shift!(nlsf_interpolation_flag, 1) == 0 {
                ...
                for i in 0..lag as usize + 5 / 2 {
                    ps_dec.sLTP_Q16[s_ltp_buf_idx - i - 1] = skp_s_mul_w_b!(
                        inv_gain_q32,
                ...
                for i in 0..lag as usize + 5 / 2 {
                    ps_dec.sLTP_Q16[(s_ltp_buf_idx - i - 1) as usize] = skp_s_mul_w_w!(
                        gain_adj_q16,
                        ps_dec.sLTP_Q16[(s_ltp_buf_idx - i - 1) as usize]
                    );
                }
            }
        }
        for i in 0..16 {
            ps_dec.sLPC_Q14[i] = skp_s_mul_w_w!(gain_adj_q16, ps_dec.sLPC_Q14[i]);
        }
        ...
        if sig_type == 0 {
            ...
            for i in 0..ps_dec.subfr_length as usize {
                ...
                ps_dec.sLTP_Q16[s_ltp_buf_idx as usize] = p_res_q10[i as usize] << 6;
                s_ltp_buf_idx += 1;
            }
        } else {
            for i in 0..ps_dec.subfr_length as usize {
        ...
        for i in 0..ps_dec.subfr_length as usize {
            ps_dec.outBuf[pxq + i] = skp_sat_16!(
    ...
    for i in 0..ps_dec.frame_length as usize {
        xq[i] = ps_dec.outBuf[ps_dec.frame_length as usize + i];
    }
}
