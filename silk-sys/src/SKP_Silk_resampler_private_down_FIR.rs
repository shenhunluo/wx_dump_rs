...
fn skp_silk_resampler_private_down_fir_interpol1<'a>(
    ...
    for index_q16 in (0..max_index_q16).step_by(index_increment_q16) {
        ...
        let interpol_ptr = &fir_coefs[(RESAMPLER_DOWN_ORDER_FIR / 2 * interpol_ind) as usize..];
...
pub fn skp_silk_resampler_private_down_fir(
    ...
    let index_increment_q16 = ss.inv_ratio_q16 as usize;
    loop {
        n_samples_in = if len < ss.batch_size as usize {
            len
        } else {
            ss.batch_size as usize
        };
        if ss.input2x == 1 {
            skp_silk_resampler_down2(&mut ss.s_down2, &mut buf1, in_0, n_samples_in);
    ...
    for i in 0..12 {
        ss.s_fir[i] = buf2[n_samples_in + i];
    }
}
