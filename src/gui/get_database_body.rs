...
use iced::{widget::{Container, Column, Button, Row, Space}, Length};
use iced_runtime::Command;
use tokio::sync::watch::{Sender, Receiver};

use crate::action::get_database::get_database;

use super::{Message, config_body::ConfigBody, show_user_info_body::ShowUserInfoBody, gui_util::{PrintInfoText, set_col_with_text_input, set_col_with_text}};
...
impl GetDatabaseBody {
    pub fn new() -> GetDatabaseBody {
        ...
        Self {
            ...
            wechat_path: None,
            print_info_text: Arc::new(PrintInfoText::new(iced::widget::scrollable::Id::new("get_database_print_info"))),
        }
    }
    ...
    pub fn update(&mut self, msg: GetDatabaseMessage, config: &ConfigBody, show_user_info_body: &ShowUserInfoBody) -> Command<Message> {
        match msg {
            GetDatabaseMessage::ButtonStart => {
                ...
                iced::Command::perform(async move {
                    let r: Result<(), anyhow::Error>= get_database(&wechat_path, &save_dir, &account, |s| print_info.push_new_len(s), |mut keys| {
                    ...
                    Message::GetDatabaseMessage(GetDatabaseMessage::CommandFinished(r.map_err(|e| e.to_string())))
                }, |msg| msg)
            },
            ...
            GetDatabaseMessage::ButtonSelectDir => {
                self.select_dir_index_tx.send(self.select_dir.parse().unwrap()).unwrap();
                iced::Command::none()
            },
        ...
        Container::new(
            {
                ...
                col = set_col_with_text(col, "备份文件夹", None::<Message>, &self.save_path, "", &Some("注意：备份操作将清空该文件夹！！！"));
                col = set_col_with_text(col, "微信文件夹", None::<Message>, self.wechat_path.as_ref().unwrap_or(&"默认文件夹".to_owned()), "", &None::<String>);
                col = set_col_with_text_input(col, "用户名", |s| GetDatabaseMessage::InputAccount(s), None, &self.account, "", &None::<String>);
                col = col.push(
                    self.print_info_text.draw().height(Length::Fill)
                );
                if *self.need_select_dir.read().unwrap() {
                    col = set_col_with_text_input(col, "请选择", |s| GetDatabaseMessage::InputSelectDir(s), self.select_dir.parse::<usize>().ok().map(|_| GetDatabaseMessage::ButtonSelectDir), &self.select_dir, "确定", &None::<String>);
                }
                col = col.push(
                    Row::new().spacing(5)
                        ...
                        .push(
                            if self.command_running {
                                ...
                                Button::new("开始备份").on_press(Message::GetDatabaseMessage(GetDatabaseMessage::ButtonStart))
                            }
                        )
                    );
