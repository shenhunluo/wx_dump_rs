...
use iced::{widget::{Container, Column, Row, Space, Button}, Length};

use crate::{util::{self, string_to_u8_vec, u8_to_string}, action::decrypt::decrypt};

use super::{config_body::ConfigBody, show_user_info_body::ShowUserInfoBody, Message, gui_util::{PrintInfoText, set_col_with_text, set_col_with_text_input}};
...
impl DecryptBody {
    pub fn new() -> Self {
        Self {
            ...
            print_info_text: Arc::new(PrintInfoText::new(iced::widget::scrollable::Id::new("decrypt_body_print_info"))),
    ...
    pub fn update(&mut self, msg: DecryptMessage, config: &ConfigBody, show_user_info: &ShowUserInfoBody) -> iced::Command<Message> {
        match msg {
            ...
            DecryptMessage::ButtonStart => {
                match &self.key {
                    Some(key) => {
                        ...
                        iced::Command::<Message>::perform(async move {
                            match decrypt(&save_path,&decrypt_path,&key,false, |s| {
                                print_info_text.push_new_len(s);
                            }) {
                                Ok(_) => Message::DecryptMessage(DecryptMessage::CommandFinished(Ok(()))),
                                Err(e) => Message::DecryptMessage(DecryptMessage::CommandFinished(Err(e.to_string()))),
                            }
                        }, |msg| msg)
                    },
                    None => {
            ...
            DecryptMessage::ButtonBase64Key => {
                match string_to_u8_vec(&self.input_base64_key.trim().to_owned(), &"base64".to_string()) {
                    Ok(s) => {
                        if s.len() != 32 {
                    ...
                    Err(e) => self.input_base64_key_err_msg = Some(format!("key格式化失败,错误信息:{}",e.to_string())),
                }
                iced::Command::none()
            }
            ...
            DecryptMessage::ButtonHexKey => {
                match string_to_u8_vec(&self.input_hex_key.trim().to_owned(), &"hex".to_string()) {
                    ...
                    Err(e) => self.input_hex_key_err_msg = Some(format!("key格式化失败,错误信息:{}",e.to_string())),
                }
                iced::Command::none()
            }
        ...
        Container::new(
            {
                ...
                col = set_col_with_text(col, "备份文件夹", None::<Message>, &self.save_path, "", &None::<String>);
                col = set_col_with_text(col, "解密文件夹", None::<Message>, &self.decrypt_path, "", &None::<String>);
                col = set_col_with_text(
                    col, 
                    "key", 
                    None::<Message>, 
                    &self.key.map(|s| s.iter().map(|u| u.to_string()).collect::<Vec<_>>().join(",")).unwrap_or("".to_owned()), 
                    "", 
                    &None::<String>
                );
                    ...
                    Row::new().spacing(5)
                        ...
                        .push(
                            if self.command_running {
                                ...
                                Button::new("开始解密").on_press(Message::DecryptMessage(DecryptMessage::ButtonStart))
                            }
                        )
                    );
