...
pub fn get_audio_pcm(
    ...
    let m: Vec<Media> = schema_media_msg::Media::table
        .select(schema_media_msg::Media::all_columns)
        ...
        match data[0] {
            2 => {
                ...
                if m.key.as_ref().unwrap() == "1099511686541" {
                    let t = crate::gui::gui_util::play_audio(|sample_rate| {
                        Ok(silk_sys::decode_silk(&data[1..], sample_rate)?)
                    })?;
                    stream = Some(t);
            ...
            35 => {
                if stream.is_none() {
                    let t = crate::gui::gui_util::play_audio(|sample_rate| {
                        Ok(silk_sys::decode_silk(&data[7..], sample_rate)?)
                    })?;
                    stream = Some(t);
                    println!("play audio. key: {}", m.key.unwrap());
                } else {
